...
def make_ortholog_file(cur, genome, intron_class, introns):
    ...
    # because it is waaaaaaaaaaaaaay faster
    # also put intron IDs in single quotes to escape them
    introns = ','.join([f"('{intron}')" for intron in introns])
    query += f'IN (VALUES {introns})'
    cur.execute(query)
    id_pairs = cur.fetchall()
    id_pairs = [
        # strip {}s from cluster numbers
        [pair[0], pair[1].lstrip('{').rstrip('}')]
        # ignore introns that were in no clusters
        for pair in id_pairs if pair[1] != '{}'
    ]
    # get a list of all the rows of orthologs we need so we only do one query
